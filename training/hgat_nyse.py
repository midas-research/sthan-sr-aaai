...
class Attention(torch.nn.Module):
    ...
    def __init__(self, dimensions, attention_type='general'):
        ...
        self.ae = torch.nn.Parameter(torch.FloatTensor(1737,1,1)) #1737 is number of stocks
        self.ab = torch.nn.Parameter(torch.FloatTensor(1737,1,1))
    ...
    def forward(self, query, context):
        ...
        delta_t = delta_t.repeat(1737,1).reshape(1737,1,query_len) #1737 is number of nodes
...
class HGAT(torch.nn.Module):
    ...
    def forward(self,price_input,e):
        ...
        query = query.reshape(1737,1,32) #1737 is number of nodes
        output, weights = self.attention(query, context)
        output = output.reshape((1737,32))
