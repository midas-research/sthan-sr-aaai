...
from load_data_tse import load_EOD_data, load_relation_data
...
class ReRaLSTM:
    ...
    def train(self):
        ...
        optimizer_hgat = optim.Adam(model.parameters(), 
                       ...
                       weight_decay=5e-4)
        inci_mat = np.load('tse.npy')
        inci_sparse = sparse.coo_matrix(inci_mat)
        incidence_edge = utils.from_scipy_sparse_matrix(inci_sparse)#.to(device)

        batch_offsets = np.arange(start=0, stop=self.test_index, dtype=int)
        for i in range(self.epochs):
            ...
            for j in tqdm(range(self.valid_index - self.parameters['seq'] -self.steps +1)):
                ...
                cur_loss, cur_reg_loss, cur_rank_loss, curr_rr_train = trr_loss_mse_rank(output.reshape((95,1)), torch.FloatTensor(price_batch).to(device), 
            ...
            with torch.no_grad():
                ...
                for cur_offset in range(
                    ...
                    output_val = model(torch.FloatTensor(emb_batch).to(device), incidence_edge[0].to(device))
                    cur_loss, cur_reg_loss, cur_rank_loss, cur_rr = trr_loss_mse_rank(output_val, torch.FloatTensor(price_batch).to(device), 
                                                                                            torch.FloatTensor(gt_batch).to(device), 
                                                                                            torch.FloatTensor(mask_batch).to(device), 
                                                                                            self.parameters['alpha'], self.batch_size)
            
                    cur_rr = cur_rr.detach().cpu().numpy().reshape((95,1))
                ...
                for cur_offset in range(self.test_index - self.parameters['seq'] - self.steps + 1,
                    ...
                    output_test = model(torch.FloatTensor(emb_batch).to(device), incidence_edge[0].to(device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', help='path of EOD data',
                        default='/content/tokyo_data/pre_processed_data/')
    ...
    parser.add_argument('-r', default=1e-3,
                        help='learning rate')
    ...
    RR_LSTM = ReRaLSTM(
        ...
        steps=1, epochs=70, batch_size=None, gpu=args.gpu,
